...
const WhatsAppMessagePreview = ({ message, className = "" }: WhatsAppMessagePreviewProps) => {
  ...
  return (
    <div
      className={`rounded-2xl border border-qit-purple/15 bg-white/80 backdrop-blur p-2.5 sm:p-4 max-w-sm ${className}`}
    >
      ...
      <div className="relative rounded-xl rounded-tl-sm bg-qit-beige px-3 py-2">
        <p className="text-xs sm:text-sm text-qit-purple/90 leading-snug whitespace-pre-line">{message}</p>
